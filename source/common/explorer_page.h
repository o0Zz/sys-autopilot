...
static const char kExplorerHtml[] =
    ...
    "};\n"
    "\n"
    "// Ctrl+S (Cmd+S on macOS) saves the open file instead of the browser's\n"
    "// \"save page as\" dialog.\n"
    ...
    "  if (!confirm('Reboot the console? The explorer is unreachable until it boots back.')) return;\n"
    "  if (dirty && !confirm('Discard unsaved changes?')) return;\n"
    "  const r = await fetch('/power/restart', { method: 'POST' });\n"
    "  if (!r.ok) return say('reboot failed: ' + (await r.text()).slice(0, 120), true);\n"
    "  say('rebooting...', true);\n"
    "};\n"
    "\n"
